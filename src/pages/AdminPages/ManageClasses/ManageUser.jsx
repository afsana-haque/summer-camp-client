...
const ManageUser = () => {
    const { data: users = [], refetch } = useQuery(['users'], async () => {
        const res = await fetch('https://music-school-server-bay.vercel.app/users')
        return res.json();
    })

    const handleManageAdmin = user => {
        fetch(`https://music-school-server-bay.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
        .then(res => res.json())
        .then(data => {
        ...
        const handleManageInstructor = user => {
            fetch(`https://music-school-server-bay.vercel.app/users/instructor/${user._id}`, {
                method: 'PATCH'
            })
            .then(res => res.json())
            .then(data => {
